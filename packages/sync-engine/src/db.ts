...
function createSqlJsAdapter(database: SqlJsDatabase): LocalDatabase {
  const adapter: LocalDatabase = {
    ...
    async getFirstAsync<T>(sql: string, params: unknown[] = []) {
      const rows = await adapter.getAllAsync<T>(sql, params);
      return rows[0] ?? null;
    },
    async transaction<T>(callback: (db: LocalDatabase) => Promise<T>) {
...
async function openDefaultDatabase(): Promise<LocalDatabase> {
  ...
  const sqlJsModule = (await import('sql.js')) as { default: (config?: Record<string, unknown>) => Promise<SqlJsStatic> };
